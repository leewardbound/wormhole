...
test_backend_coverage:
	pytest --cov=wormhole/apps --cov-config=.coveragerc --cov-report html --cov-report term
...
isort:
	pipenv run isort wormhole

flake8:
	pipenv run flake8

autoflake:
	pipenv run autoflake -r -i --expand-star-imports --remove-all-unused-imports --remove-duplicate-keys --remove-unused-variables --ignore-init-module-imports wormhole/apps/

black:
	pipenv run black wormhole wsgi.py manage.py

lint:
	pipenv run black --check wormhole wsgi.py manage.py
